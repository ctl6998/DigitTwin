...
class my_yolov6():
    ...
    def infer(self, source, target_class='person', conf_thres=0.25, iou_thres=0.45, classes=None, agnostic_nms=False, max_det=1000):
        ...
        if len(det):
            ...
            for *xyxy, conf, cls in reversed(det):
                class_num = int(cls)  # integer class
                if class_num == target_class_idx:
                    person_detections += 1
                    label = f'{self.class_names[class_num]} {conf:.2f}'
                    self.plot_box_and_label(img_src, max(round(sum(img_src.shape) / 2 * 0.003), 2), xyxy, label,
                                            color=(255, 0, 0))
            img_src = np.asarray(img_src)

        return img_src, person_detections